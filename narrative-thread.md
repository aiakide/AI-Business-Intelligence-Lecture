...
**✅ Diagramme ergänzt (2026-09-02, User-Wunsch):** User bemängelte, dass Neuron-Aufbau, Netzwerk-Architektur, Datenfluss und Gewichtsupdate nur in ASCII-Art/Prosa erklärt wurden. `diagram-generator` erstellte 4 neue SVGs im etablierten FOM-Stil (`#00C6B2`, Versicherer-Case durchgängig): `neuron-aufbau-diagramm.svg` (gewichtete Summe + Aktivierung in 2 Schritten), `netzwerk-schichten-diagramm.svg` (Input/Hidden/Output vollvernetzt, wiederverwendet auf 2 Folien mit unterschiedlicher Hervorhebung), `forward-pass-diagramm.svg` (Datenfluss mit konkreten Zahlen [25,3,1]→66%), `backprop-zyklus-diagramm.svg` (Trainings-Kreislauf im CRISP-DM-Stil: Forward→Loss→Backward→Update). 5 Folien von `default` auf `header-cols` umgestellt, um Text+Diagramm nebeneinander zu zeigen. **Incident:** Beim Umstellen einer Folie blieb ein verwaister `layout: default`-Block stehen, was eine unsichtbare Phantom-Folie erzeugte UND das Diagramm auf der eigentlichen Folie unterdrückte (Slidev interpretierte die doppelte Frontmatter als zwei Folien-Trenner) — von der scrollHeight-Metrik nicht erkannt, nur per Screenshot sichtbar. Behoben, re-verifiziert clean. Kapitel 6 hat dadurch jetzt 28 Content-Folien (Phantom-Folie durch Fix wieder raus).

**✅ Weitere Diagramme + Feinschliff (2026-09-02, User-Feedback):**
- User bemängelte, dass alle 5 Hidden-Neuronen im Netzwerk-Diagramm identisch mit σ beschriftet waren ("selbes Symbol") — wirkte wie eine Wiederholung statt 5 eigenständiger Neuronen. Fix: σ-Labels von den Hidden-Neuronen entfernt (jetzt leere Kreise, Standard-Konvention), Output-Neuron behält sein σ-Label zur Unterscheidung.
- User bemängelte einen unnötigen Erklärsatz auf der Output-Schicht-Folie ("Das Diagramm rechts zeigt..."). Entfernt.
- User wünschte zusätzlich Diagramme für die Aktivierungsfunktionen. `diagram-generator` erstellte 4 weitere SVGs: `sigmoid-kurve-diagramm.svg` und `relu-kurve-diagramm.svg` und `tanh-kurve-diagramm.svg` (Funktionsgraphen mit visuell markierten Problemzonen — Vanishing-Gradient-Sättigung bzw. Dead-Neuron-Zone), `softmax-balken-diagramm.svg` (Balkendiagramm Raw-Output vs. Wahrscheinlichkeitsverteilung, exakt die Zahlen aus dem Text). 4 Folien (Sigmoid/ReLU/Tanh/Softmax) von `default` auf `header-cols` umgestellt.
- **QA-Historie:** Ein Overflow auf der Sigmoid-Folie brauchte 2 Nachbesserungsrunden — die erste Kürzung reduzierte den scrollHeight-Overflow, aber der Text kollidierte weiterhin mit der (offenbar fix positionierten) `<LiteraturSource>`-Quellenzeile, was von scrollHeight/clippedElements-Metriken NICHT erfasst wurde (nur per expliziter Bounding-Box-Messung sichtbar). Erst nach deutlicherer Kürzung (2 knappe Bullets + 1 kurzer Satz) war die Kollision behoben (24px Abstand verifiziert). **Lektion:** Bei `LiteraturSource`-Footer-Kollisionen reicht die scrollHeight-Metrik nicht — explizite Bounding-Rect-Messung von letztem Content-Element vs. Quellenzeile nötig.
- Alle Änderungen visuell verifiziert, kein Overflow mehr in Kapitel 6.

**✅ Weitere Diagramme + Reihenfolge-Korrektur (2026-09-02, User-Feedback):**
- User bemängelte, dass in `forward-pass-diagramm.svg` (separate Datei, beim ersten Fix übersehen) dieselben identischen σ-Labels auf den Hidden-Neuronen auftauchten wie zuvor bei `netzwerk-schichten-diagramm.svg`. Gleicher Fix angewendet (Labels entfernt).
- User wünschte zusätzlich Diagramme für Loss, Gradient Descent und Backpropagation. `diagram-generator` erstellte: `loss-kurve-diagramm.svg` (sinkende Lernkurve über Trainingsschritte), `gradient-descent-diagramm.svg` (schüsselförmige Loss-Landschaft mit Abstiegsschritten, visualisiert die "Wanderer im Nebel"-Metapher), `backprop-fluss-diagramm.svg` (Netzwerk-Fluss mit sichtbar getrenntem Forward-Pass/Backward-Pass, Loss als Scharnierpunkt). 3 Folien angepasst (2× auf header-cols umgestellt, 1× Diagramm ersetzt ASCII-Block).
